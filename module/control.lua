...
local function collect_and_send_rail_entities()
	...
	for _, surface in pairs(game.surfaces) do
		...
		for _, entity in ipairs(entities) do
			if entity.valid and RAIL_TYPES[entity.type] and entity.type ~= "entity-ghost"
				and not UE_PROTECTED_NAMES[entity.name] then
				local entry = {
					name      = entity.name,
					type      = entity.type,
...
local function collect_and_send_ue_stops()
	local config = storage.gridworld
	if config == nil or config.is_pathworld then return end
...
function gridworld.apply_rail_entities(json)
	...
	for _, surface in pairs(game.surfaces) do
		...
		for _, entity in ipairs(existing) do
			if entity.valid and RAIL_TYPES[entity.type] and not UE_PROTECTED_NAMES[entity.name] then
...
function gridworld.apply_ue_stops(json)
...
local function destroy_if_outside(entity)
	...
	if UE_PROTECTED_NAMES[entity.name] then
		return
	end
...
gridworld.on_nth_tick[900] = function()
	collect_and_send_rail_entities()
	collect_and_send_ue_stops()
end
...
gridworld.events[defines.events.on_chunk_generated] = function(event)
	...
	if config.is_pathworld then
		...
		for _, entity in ipairs(entities) do
			if entity.valid and entity.type ~= "character" and not RAIL_TYPES[entity.type]
				and not UE_PROTECTED_NAMES[entity.name] then
				entity.destroy()
			end
		end
	...
	for _, entity in ipairs(entities) do
		if entity.valid and entity.type ~= "character" then
			if not UE_PROTECTED_NAMES[entity.name] and is_outside_bounds(entity.position, bounds) then
				entity.destroy()
			end
		end
	end
